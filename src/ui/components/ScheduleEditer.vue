...
import { editableDays, isDisplaySpecialDay } from "~/presentation/presenters/day";
...
export default defineComponent({
  ...
  setup(_, { emit }) {
    const updateModule = (index: number, schedule: EditableSchedule, module: EditableModule) => {
      emit("update:schedules", index, { ...schedule, module });
    };

    const updateDay = (index: number, schedule: EditableSchedule, day: EditableDay) => {
      if (isDisplaySpecialDay(day)) emit("update:schedules", index, { module: schedule.module, day });
      else
    ...
    const updatePeriod = (index: number, schedule: EditableSchedule, period: EditablePeriod) => {
      if ("period" in schedule) emit("update:schedules", index, { ...schedule, period });
    };
...
<template>
  <div class="schedule-editer">
    <section v-for="(schedule, index) in schedules" :key="index" class="schedule-editer__row">
      <div class="schedule-editer__container">
        <Dropdown
          :selectedOption="schedule.module"
          :options="editableModules"
          :label="index > 0 ? '' : '学期'"
          @update:selectedOption="(module) => updateModule(index, schedule, module)"
        ></Dropdown>
        <Dropdown
        ...
        ></Dropdown>
        <Dropdown
          :selectedOption="'period' in schedule ? schedule.period : notSpecified"
          :options="isDisplaySpecialDay(schedule.day) ? [notSpecified] : editablePeriods"
          :label="index > 0 ? '' : '時限'"
          :state="isDisplaySpecialDay(schedule.day) ? 'disabled' : 'default'"
          @update:selectedOption="(period) => updatePeriod(index, schedule, period)"
        ></Dropdown>
      </div>
      <div v-if="index > 0" class="schedule-editer__row__remove" @click="onClickRemoveButton(index)">
        <span class="material-icons">close</span>
      </div>
    </section>
    <div class="schedule-editer__add" @click="onClickAddButton"><span class="material-icons">add</span>追加する</div>
  </div>
</template>
