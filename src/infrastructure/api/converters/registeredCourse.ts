...
import { apiToSchedules, isEqualCourseSchedules, schedulesToApi } from "./schedule";

/**
 * Convert registerd course from api type to domain type.
 */
export const apiToRegisteredCourse = (apiCourse: Omit<ApiType.RegisteredCourse, "userId">): RegisteredCourse => {
  const { schedules, rooms } = apiToSchedules(apiCourse.schedules ?? apiCourse.course?.schedules ?? []);
  ...
  const course: RegisteredCourse = {
    ...
    instructors: apiToInstructors(apiCourse.instructor ?? apiCourse.course?.instructor ?? ""),
 ...
 */
const updateApiCourseProp = <P extends keyof ApiType.Course & keyof Required<ApiType.RegisteredCourse>>(
  apiCourse: ApiType.RegisteredCourse,
  prop: P,
  newValue: Required<ApiType.RegisteredCourse>[P],
  compareFn: (v1: ApiType.Course[P], v2: Required<ApiType.RegisteredCourse>[P]) => boolean
): void => {
...
export const registeredCourseToApi = (
  ...
  const schedules: ApiType.CourseSchedule[] = schedulesToApi(course.schedules, course.rooms);
  ...
  updateApiCourseProp(apiCourse, "methods", course.methods, isEqualCourseMethods);
  updateApiCourseProp(apiCourse, "schedules", schedules, isEqualCourseSchedules);
