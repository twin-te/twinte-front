...
export class CalendarRepositoryInMemory implements ICalendarRepository {
  ...
  constructor() {
    this.#events = ([...event_2021, ...event_2022] as ApiType.SchoolCalendarEvent[])
      .map(apiToEvent)
      .filter((event): event is Event => event != undefined);

    this.#modules = ([...module_2021, ...module_2022] as ApiType.SchoolCalendarModule[]).map(apiToModuleInformation);
  }
  ...
  async getModuleInformationList(
    ...
    const moduleInformationList = this.#modules.filter((info) => info.year === year);
