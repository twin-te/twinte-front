...
import { apiToEvent, apiToModuleInformation } from "~/infrastructure/api/converters/calendar";
...
export class CalendarRepository implements ICalendarRepository {
  ...
  async getEventsByYear(year: number): Promise<Event[] | UnauthorizedError | NetworkError | InternalServerError> {
    if (this.#years.event.has(year)) {
      const events = this.#events.filter((event) => getAcademicYear(event.date) === year);

      return events;
    }

    return this.#api.call<ApiType.SchoolCalendarEvent[], Event[], 200, 400 | 401 | 500>(
      (client) => client.school_calendar.events.get({ query: { year } }),
      (body) => {
        const events = body.map(apiToEvent).filter((event): event is Event => event != undefined);
  ...
  async getModuleInformationList(
    ...
    if (this.#years.module.has(year)) {
      const moduleInformationList = this.#modules.filter((module) => module.year === year);

      return moduleInformationList;
    }

    return this.#api.call<ApiType.SchoolCalendarModule[], ModuleInformation[], 200, 400 | 401 | 500>(
