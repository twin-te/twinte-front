...
export const isContained = <T extends string | number | boolean>(value: unknown, array: T[]): value is T => {
  return array.findIndex((el) => el === value) !== -1;
};

export const initializeObject = <K extends string | number | symbol, V>(keys: K[], initValue: V): Record<K, V> => {
  return keys.reduce((obj, key) => ({ ...obj, [key]: deepCopy(initValue) }), {} as Record<K, V>);
};

export const getKeysFromObj = <T extends object>(obj: T) => Object.keys(obj) as (keyof T)[];

export const removeDuplicate = <T>(array: T[]): T[] => new Array(...new Set(array));

export const isEqualPrimitive = <T extends string | number | boolean>(v1: T, v2: T): boolean => v1 === v2;
...
export const isBetween = (target: Dayjs, start: Dayjs, end: Dayjs, unit?: OpUnitType): boolean => {
  return (
 ...
 */
export const containedInArray = <T extends { id: string }>(array: T[], element: T): boolean => {
  return array.findIndex(({ id }) => id === element.id) !== -1;
};

export const addElementsInArray = <T extends { id: string }>(array: T[], newElements: T[]) => {
 ...
 */
export const updateAllElementsInArray = <T extends { id: string }>(array: T[], newElements: T[]) => {
 ...
 */
export const updateElementInArray = <T extends { id: string }>(array: T[], newElement: T) => {
 ...
 */
export const deleteElementInArray = <T extends { id: string }>(array: T[], id: string) => {
  const index = array.findIndex((element) => element.id === id);
  if (index !== -1) array.splice(index, 1);
};

const hasUnknownProperty = <P extends string>(obj: object, prop: P): obj is { [key in P]: unknown } => {
  return prop in obj;
};
