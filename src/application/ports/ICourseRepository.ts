...
import { InternalServerError, NetworkError, NotFoundError, UnauthorizedError, ValueError } from "~/domain/error";
...
export interface ICourseRepository {
  ...
  getAllTags(): Promise<Tag[] | UnauthorizedError | NetworkError | InternalServerError>;

  getTagById(id: string): Promise<Tag | NotFoundError | UnauthorizedError | NetworkError | InternalServerError>;

  /**
   * Create the new tag of which order is set to last.
   */
  createTag(name: string): Promise<Tag | UnauthorizedError | NetworkError | InternalServerError>;
  ...
  deleteTag(id: string): Promise<null | NotFoundError | UnauthorizedError | NetworkError | InternalServerError>;
}
